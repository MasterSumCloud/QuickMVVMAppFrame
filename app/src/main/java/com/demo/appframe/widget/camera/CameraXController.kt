...
class CameraXController(
    fragmentActivity: FragmentActivity, private var preview: PreviewView?, private var callBack: CameraCallBack?
) : ICameraController {
        ...
        cameraProviderFuture?.addListener({
            ...
            if (cameraParams?.mRatioType == CameraRatioType.RATIO_1_1 || cameraParams?.mRatioType == CameraRatioType.RATIO_FULL) {
                ...
                mPreView = Preview.Builder()
                    //设置分辨率
                    .setTargetResolution(size).build().also {
                ...
                mPreView = Preview.Builder()
                    //设置分辨率
                    .setTargetAspectRatio(cameraParams?.mRatioType!!).build().also {
                        it.setSurfaceProvider(preview?.surfaceProvider)
                    }

                //图像捕捉
                imageCapture =
                    ImageCapture.Builder().setTargetAspectRatio(cameraParams?.mRatioType!!).setCaptureMode(ImageCapture.CAPTURE_MODE_MAXIMIZE_QUALITY)
                        .build()
...
//                //---------------------------------------------CameraX 高级用法 几乎国内设备都不支持扩展-------------------------------------
            ...
            mLifecycleOwner?.lifecycleScope?.launch {
                val extensionsManager = ExtensionsManager.getInstanceAsync(mLifecycleOwner!!, cameraProvider!!).get()

                if (extensionsManager.isExtensionAvailable(
                        cameraSelector, cameraParams?.extensionMode!!
                    )
                ) {
    ...
    private fun bindCameraId(cameraSelector: CameraSelector) {
        try {
            ...
            camera = cameraProvider?.bindToLifecycle(
                mLifecycleOwner!!, cameraSelector, mPreView, imageCapture, videoCapture
...
//        val name = SimpleDateFormat("yyyy-MM-dd", Locale.CHINA).format(System.currentTimeMillis())
        /*val contentValues = ContentValues().apply {
            put(MediaStore.MediaColumns.DISPLAY_NAME, "SY-CAMERA-${TimeUtils.getNowMills()}")
            put(MediaStore.MediaColumns.MIME_TYPE, "image/jpeg")
            if (Build.VERSION.SDK_INT > Build.VERSION_CODES.P) {
                put(MediaStore.Images.Media.RELATIVE_PATH, "Pictures/CameraX-Image")
            }
        }*/
        val savePath = "${PathUtils.getExternalAppCachePath()}/sy-camera/take-photo-temp${TimeUtils.getNowMills()}.jpeg"
        val saveFile = File(savePath)
        saveFile.mkdirs()
        val outputOptions = ImageCapture.OutputFileOptions.Builder(saveFile).build()
        imageCapture.takePicture(outputOptions, ContextCompat.getMainExecutor(mLifecycleOwner!!), object : ImageCapture.OnImageSavedCallback {
            override fun onError(exc: ImageCaptureException) {
                callBack?.takePictureStatus(false, exc.message.toString())
            }

            override fun onImageSaved(output: ImageCapture.OutputFileResults) {
                callBack?.takePictureStatus(true, output.savedUri.toString(), output.savedUri)
            }
        })
    }
    ...
    override fun takeVideo() {
        ...
        val mediaStoreOutputOptions = mLifecycleOwner?.contentResolver?.let {
            MediaStoreOutputOptions.Builder(it, MediaStore.Video.Media.EXTERNAL_CONTENT_URI).setContentValues(contentValues).build()
        }
        recording = videoCapture.output.prepareRecording(mLifecycleOwner!!, mediaStoreOutputOptions!!).apply {
            if (ActivityCompat.checkSelfPermission(
                    mLifecycleOwner!!, Manifest.permission.RECORD_AUDIO
            ...
            when (recordEvent) {
                is VideoRecordEvent.Start -> {
                    //录制开始
                    Toast.makeText(mLifecycleOwner, "开始录制", Toast.LENGTH_SHORT).show()
                }
                ...
                is VideoRecordEvent.Finalize -> {
                    ...
                    if (!recordEvent.hasError()) {
                        ...
                        Log.e(
                            TAG, "Video capture ends with error: " + "${recordEvent.cause?.message}"
    ...
    override fun focus(x: Float, y: Float, auto: Boolean) {
        ...
        val createPoint: MeteringPoint = if (auto) {
            ...
            val meteringPointFactory = DisplayOrientedMeteringPointFactory(
                preview?.display!!, camera?.cameraInfo!!, preview?.width?.toFloat()!!, preview?.height?.toFloat()!!
            )
        ...
        val build = FocusMeteringAction.Builder(createPoint, FLAG_AF).setAutoCancelDuration(3, TimeUnit.SECONDS).build()
